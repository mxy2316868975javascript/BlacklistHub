...
export async function GET(request: NextRequest) {
  ...
  if (start || end) {
    ...
    (q as unknown as { created_at: { $gte?: Date; $lte?: Date } }).created_at = range;
  }
  if (keyword) q.$or = [
  ...
  const [items, total] = await Promise.all([
    Blacklist.find(q).sort({ updated_at: -1 }).skip((page - 1) * pageSize).limit(pageSize).lean(),
...
export async function POST(request: NextRequest) {
  ...
  const { type, value, reason, reason_code, risk_level, source, expires_at } = body || {};
  if (!type || !value || !reason || !reason_code || !risk_level)
    return NextResponse.json({ message: "缺少参数: 请提供 类型/值/风险等级/理由码/原因摘要" }, { status: 400 });
  ...
  if (existing) {
    ...
    const oldTimeline = (existing.timeline as unknown as { action: string; by: string; at: Date; note?: string }[]) || [];
    updates.timeline = [
      ...oldTimeline,
      { action: "merge_source", by: me.username, at: new Date(), note: source ? `merge source: ${source}` : undefined },
    ];
    const doc = await Blacklist.findByIdAndUpdate(existing._id, { $set: updates }, { new: true }).lean();
    return NextResponse.json({ merged: true, doc });
  }

  const exp = expires_at ? new Date(expires_at) : new Date(Date.now() + 180 * 24 * 60 * 60 * 1000);
  const doc = await Blacklist.create({
    ...
    status: "draft",
    timeline: [
      { action: "create", by: me.username, at: new Date() },
    ],
  });
