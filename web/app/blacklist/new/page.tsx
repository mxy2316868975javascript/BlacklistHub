...
export default function NewBlacklistPage() {
  ...
  const valuePlaceholder = (() => {
    switch (typeValue) {
      case "email": return "例如: user@example.com";
      case "ip": return "例如: 192.168.1.1";
      case "phone": return "例如: 13812345678";
      case "domain": return "例如: example.com";
      default: return "请输入标的值";
    }
  })();
    ...
    ({ getFieldValue }: any) => ({
      validator(_: unknown, v: string) {
        ...
        if (t === "ip") {
          ...
          return ok ? Promise.resolve() : Promise.reject("请输入有效 IPv4 地址");
        }
        if (t === "domain") {
          const ok = /^(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,}$/i.test(v);
      ...
      <Card className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-neutral-500 text-sm">填写必要字段，保存后可在详情页继续流转</p>
          </div>
          <Button type="primary" onClick={() => form.submit()}>保存</Button>
        </div>
        <div className="my-4 border-t" />
        <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ type: "user", risk_level: "medium" }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Form.Item name="type" label="类型" rules={[{ required: true }]}>
            <div className="text-neutral-500 text-xs -mt-2 mb-2"><InfoCircleOutlined className="mr-1" />例如：abuse.spam、fraud.chargeback</div>

              <Select options={[{ label: "用户", value: "user" }, { label: "IP", value: "ip" }, { label: "邮箱", value: "email" }, { label: "手机号", value: "phone" }, { label: "域名", value: "domain" }]} />
            </Form.Item>
            ...
            <Form.Item name="reason_code" label="理由码" rules={[{ required: true }]}>
          ...
          <Form.Item name="reason" label="原因摘要" rules={[{ required: true, min: 5 }]}>
