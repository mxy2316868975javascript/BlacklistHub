...
export async function GET(request: NextRequest) {
  ...
  if (!me || (me.role || "reporter") !== "admin") return NextResponse.json({ message: "Forbidden" }, { status: 403 });
  ...
  for (const s of stats as Array<{ _id: string; total: number; published: number }>) {
    statMap.set(s._id, { total: s.total, published: s.published });
  }
  const items = users.map((u) => ({ ...u, stats: statMap.get(u.username) || { total: 0, published: 0 } }));
